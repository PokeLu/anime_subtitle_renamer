...
class BaseMatcher(ABC):
    ...
    def __call__(self, video_names: List[str], sub_names: List[str]) -> Dict[str, str]:
        ...
        def match_new_sub_name(sub_name):
            new_sub_name = None
        ...
        # 获取新字幕名，multi-threading
        with ThreadPoolExecutor(max_workers=32) as executor:
            key_value_pairs = executor.map(
                lambda sub_name: match_new_sub_name(sub_name),
                sub_names
            )
            new_sub_name_dict = dict(key_value_pairs)
